...
static inline float domain_get_offset_x(const Domain* domain, float x, float z, int32_t seed)
{
     if (domain->noise_x == NULL) return 0.0f;
     return noise_compute_2d(domain->noise_x, x, z, seed) * domain->amplitude;
}

static inline float domain_get_offset_z(const Domain* domain, float x, float z, int32_t seed)
{
...
{
     return x + domain_get_offset_x(domain, x, z, seed);
}

static inline float domain_get_z(const Domain* domain, float x, float z, int32_t seed)
{
